...
- **Crop and masks:** edit percentage crop margins; rectangle, ellipse or polygon masks; inverted masks and feathered edges. Inspector geometry guides support dragging bounds and vertices. These operate in the element's drawing box before transforms; program preview shows the actual result. See [advanced editing](advanced-editing.md).
- **Transform keyframes:** animate X/Y, scale, rotation and opacity with ordered frame-based points, hold/linear/preset easing or custom Bézier curves. The inspector edits values and curve handles; canvas edits update the current key on animated properties. Cuts, trims and fps changes retain the animation clock.
- **Speed and freeze frames:** constant 0.125×–8× playback, editable speed points and easing, silent freeze holds, pitch-preserved or muted audio, and retained-original reset. Ctrl-drag a video's right edge to retime the same source interval; jobs start on release, with real progress/cancel. Generated media and recipes persist; other timeline clips retain their positions. See [speed ramps](speed-ramping.md).
- **Opacity:** Properties → Transform exposes 0–100% opacity for video, images, text, captions, graphics and annotations. MCP uses `clip.update` with `opacity: 0..1`. This affects the visual element, including its transition artwork, while its audio volume remains independent. It is a static clip setting; arbitrary opacity keyframes are not included.
...
- **Color grading:** exposure EV, contrast, saturation, temperature, tint, gamma and hue for video/image clips or a project-wide grade. MCP can target clips, tracks or timeline ranges; valid range boundaries split affected footage without moving the sequence. Clip adjustments run before the project grade, while text/graphics retain their colors. Preview and export use shared grading math; the optimized native path uses an interpolated LUT. See the [color guide](color-grading.md).
...
Audio supports trimming, splitting, imported music/sound effects, overlapping mixing, clip/master volume, mute, fades and editable gain automation. Ducking generates an editable curve from foreground clip spans; it does not detect speech or measure loudness. EQ, compression and room reverb render a project-owned audio copy while preserving original media. See [audio editing](audio-editing.md) and the [sound library](sound-library.md).
...
| `set_color_grade` | Preview/apply color adjustments to selected clips, tracks, timeline sections or the project baseline. |
| `inspect_clip_animation` | Read evaluated transform values at requested timeline frames. |
| `apply_speed_ramp` / `get_speed_job` | Retime a clip using an editable recipe or target duration; inspect/cancel real processing. |
| `reset_clip_speed` | Restore the retained source interval at normal speed. |
...
The service is intended for trusted local use. Saved projects share a workspace with one active project at a time. There is no multi-user authentication, portable single-project media package, motion tracking, advanced LUT/scopes/HDR grading, visual semantic search, waveform analysis, final-export cancellation, or desktop installer. Visual keyframes currently cover transforms and opacity; speed ramps do not generate optical-flow frames. Graphic recipes have their own independent keyframes; comparison presets are not automatic multi-clip templates. Queued jobs do not survive a server restart; project data, transcripts, original media, completed exports, and saved undo history do.
