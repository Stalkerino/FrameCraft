...
Open **Color & scopes** from the program monitor. Import standalone 3D `.cube` LUTs (grids 2–65, explicit domains supported), reuse them across projects, and choose a creative LUT plus intensity in clip or sequence grading. LUT data is embedded once in each project; no external path is needed to reopen it. A 1D shaper is rejected rather than discarded.

Corrections can run in sRGB or linear sRGB. After correction, the image returns to sRGB for the creative LUT, using trilinear interpolation. Native Vulkan evaluates the same equations and floating-point lookup tables as the browser media shader. Tagged 8/10-bit SDR input can be normalized on GPU from supported Rec.709/Rec.2020/P3 primaries and limited/full range; output remains 8-bit Rec.709 SDR. The simple native cut renderer retains its narrower input contract. Hardware validation remains pending separately on AMD/Windows, NVIDIA/Windows, AMD/Linux and NVIDIA/Linux.

Scopes capture a sampled, actually composed frame through the render queue: luma waveform, RGB parade, RGB/luma histogram and Cb/Cr vectorscope. They are explicit captures, save a PNG, and report the frame/revision so an old measurement is identifiable. Scope statistics describe display sRGB samples, not raw sensor values, broadcast signal legality or HDR nits. Statistical analysis uses a small CPU image; it does not download frames in the native video pipeline.

MCP exposes `list_color_luts`, `import_color_lut`, `attach_color_lut`, `capture_color_scopes` and `get_color_scopes`. Existing `set_color_grade` accepts `space: "srgb" | "linear-srgb"` and `lut: {id, strength}` (or `null`) for clips, tracks and ranges. Native shaders use [libplacebo custom textures](https://libplacebo.org/custom-shaders/); source normalization uses [FFmpeg libplacebo color conversion](https://www.ffmpeg.org/ffmpeg-filters.html#libplacebo).

A LUT on an entire nested sequence group is supported by native Vulkan; browser composition and scope captures cannot apply a 3D LUT to arbitrary DOM groups. Use the native monitor/export for that case, or apply the LUT to footage inside the child sequence. HDR mastering, automatic camera-log conversion, ICC display calibration, grading keyframes, secondary qualifier masks and automatic shot matching are not implemented. Camera-conversion LUTs must match the decoded input signal; no such mapping is inferred.
