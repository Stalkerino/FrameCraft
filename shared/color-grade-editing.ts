...
export function colorGradeCommands(project: Project, input: ColorGradeRequest): {commands: Command[]; affectedClipIds: string[]; splitCount: number} {
  ...
  if(body.scope === 'timeline' && !body.ranges) return {commands: [{type: 'project.color-grade', grade: merge(project.colorGrade)}], affectedClipIds: project.clips.filter(clip => ['image', 'video', 'sequence'].includes(clip.kind)).map(clip => clip.id), splitCount: 0};
  ...
  if(body.clipIds) for(const id of body.clipIds) if(!project.clips.some(clip => clip.id === id && ['image', 'video', 'sequence'].includes(clip.kind))) throw new Error('Color grading targets imported video or image clips.');
  ...
  const targets = project.clips.filter(clip => ['image', 'video', 'sequence'].includes(clip.kind)
  ...
  const fragment = (clip: Clip, start: number, end: number, id: string): Clip => {
    ...
    return {...structuredClone(clip), id, start, duration: end - start,
      sourceStart: clip.sourceStart + (clip.kind === 'video' || clip.kind === 'audio' || clip.kind === 'sequence' || clip.caption ? offset : 0),
      ...(offset ? {motionOffset: (clip.motionOffset ?? 0) + offset, audioEnvelope: shiftAudioEnvelope(clip.audioEnvelope, offset), audioDucking: shiftAudioEnvelope(clip.audioDucking, offset), transition: 'none' as const, presetTransition: null} : {})};
  };
  let splitCount = 0;
