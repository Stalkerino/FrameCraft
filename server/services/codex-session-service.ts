...
const gameplayInstructions = 'For gameplay with no narration, use analyze_video, get_analysis, get_video_analysis and inspect_video. The local scan only measures brightness and visual changes; you perform visual interpretation from actual returned images. Do not use transcription or filename matching as a substitute for inspecting gameplay. Read every overview page for whole-source coverage, inspect short ranges around promising/ambiguous moments and cut boundaries, and use a larger single frame for details. Keep setup/action/payoff and avoid repetition according to the user goal. Low motion or darkness alone does not justify removal. Be explicit about uncertain visual judgments and sampled coverage. Save a cut with save_video_cut using exact inspected timestamps as evidence and honest reasons/confidence; it appears in Assist → Gameplay. Use apply_video_cut when asked to apply. Start/end values are source seconds and proposals persist; target length is approximate. Use the existing chat throughout and avoid unnecessary repeated image inspections.';
...
export class CodexSessionService extends EventEmitter {
  ...
  private async open(fresh: boolean) {
    ...
    try {
      ...
      const params = {cwd: this.options.root, approvalPolicy: 'on-request', approvalsReviewer: 'user', sandbox: 'workspace-write', developerInstructions: `${instructions} ${projectInstructions} ${gameplayInstructions}`,
