...
export class AgentSessionService extends EventEmitter {
  ...
  constructor(private options: {root: string; data: string; url: string}) {
    ...
    for(const provider of [this.codex, this.ollama]) provider.on('change', () => {if(provider === this.active()) this.emit('change', this.snapshot());});
  }
  async init() {
  ...
  private active() {return this.settings.provider === 'ollama' ? this.ollama : this.codex;}
  ...
  async configureProvider(input: AgentProviderSettings) {
    ...
    try {
      ...
      await new AgentWorkspaceService(this.options, () => settings).validate();
  ...
  close() {this.codex.close(); this.ollama.close();}
}
