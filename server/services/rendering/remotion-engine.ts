...
export const remotionEngine: RenderEngine = {id: 'remotion-compatibility', render: renderCompatibilityProject};

/** Runs in a worker whose temporary directory is on the project's disk. */
async function renderCompatibilityProject({project, job, workspace, root, exports, mediaBase}: RenderTask, onProgress: (progress: RenderProgress) => void) {
  ...
  try {
    ...
    if(hardware) onProgress({phase: 'Preparing full composition', progress: .1, detail: browserRenderer.label,
      warning: `Compatibility export selected. ${browserRenderer.label}. This export uses frame-by-frame browser composition; source extraction, lossless frame transfer and audio use CPU. Select Native GPU · Vulkan composition in export settings to use the native video pipeline.`});
    const onDownload = () => {
    ...
    return file;
  } catch(error) {console.error('Compatibility render failed:', error instanceof Error ? error.message : String(error)); throw error;}
