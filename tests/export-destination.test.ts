...
it('scopes defaults to project storage and validates explicit destination filenames', () => {
  const file = defaultExportPath('/data', {id: 'a', name: 'Demo: ../rush'}, 'mp4');
...
it('publishes without overwriting and opens the actual custom export destination', async () => {
  ...
  try {
    ...
    expect(await resolveRenderOutput({id: 'a', kind: 'video', revision: 1, status: 'done', progress: 1, outputPath: destination, url: '/api/render/a/file'}, '/unused')).toEqual({path: destination});
