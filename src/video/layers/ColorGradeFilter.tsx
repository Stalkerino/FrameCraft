...
export function ColorGradeFilter({clip, project, children}: {clip?: ColorGrade | null; project?: ColorGrade | null; children: ReactNode}) {
  const id = `framecraft-grade-${useId().replace(/[^a-zA-Z0-9_-]/g, '')}`;
  const stages = useMemo(() => colorGradeStages(clip, project), [clip, project]);
  return <>{stages.length > 0 && <svg width="0" height="0" aria-hidden="true" style={{position: 'absolute', pointerEvents: 'none'}}><defs>
    <filter id={id} x="0" y="0" width="100%" height="100%" colorInterpolationFilters="sRGB">
      {stages.map((stage, index) => <Fragment key={index}>
        <feColorMatrix in={index ? `grade-${index - 1}` : 'SourceGraphic'} type="matrix" values={stage.matrix.join(' ')} result={`matrix-${index}`}/>
        <feComponentTransfer in={`matrix-${index}`} result={`grade-${index}`}>
