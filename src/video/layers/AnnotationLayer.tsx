...
export function AnnotationLayer({clip, width: canvasWidth, height: canvasHeight}: {clip: Clip; width: number; height: number}) {
  ...
  return <svg data-preview-clip={clip.id} width={width} height={height} viewBox={`0 0 ${width} ${height}`} style={{position: 'absolute', left: `${clip.x}%`, top: `${clip.y}%`, overflow: 'visible', opacity: progress, transform: `translate(-50%, -50%) rotate(${shape.rotation + (clip.rotation ?? 0)}deg) scale(${clip.scale * (.9 + .1 * progress)})`, filter: 'drop-shadow(0 2px 5px #0008)', ...visualGeometryStyle(clip, {width: canvasWidth, height: canvasHeight})}}>
