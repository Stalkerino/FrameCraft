...
function TextLayer({clip}: {clip: Clip}) {
  ...
  return <div data-preview-clip={clip.id} style={{position: 'absolute', left: `${clip.x}%`, top: `${clip.y}%`, transform: `translate(${translateX}, ${clip.animation === 'rise' ? (1 - progress) * 25 : 0}px) scale(${clip.scale})`, transformOrigin: `${clip.align} top`, opacity: clip.animation === 'rise' ? progress : 1, fontFamily: 'Arial, Helvetica, sans-serif', fontSize: clip.fontSize, fontWeight: Number(clip.weight), color: clip.color, textAlign: clip.align, whiteSpace: 'pre', lineHeight: 1.02, letterSpacing: clip.fontSize > 60 ? '-0.045em' : '.14em', textShadow: '0 2px 24px #00000040'}}>
...
function VisualLayer({clip, project, mediaBase, muted = false, opaque = true, pending = false, previewSource, onPreviewSourceError}: VisualProps) {
  ...
  const style = {width: '100%', height: '100%', objectFit: 'contain' as const, transform: `translate(${clip.x - 50}%, ${clip.y - 50}%) scale(${clip.scale * zoomAtFrame(clip, frame)})`, transformOrigin: `${clip.zoom?.x ?? 50}% ${clip.zoom?.y ?? 50}%`};
