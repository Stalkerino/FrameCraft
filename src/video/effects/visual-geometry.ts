...
export function visualGeometryStyle(clip: Clip, project: Pick<Project, 'width' | 'height'>): CSSProperties {
  ...
  const x = mask.x * width / 100; const y = mask.y * height / 100;
  const w = mask.width * width / 100; const h = mask.height * height / 100;
