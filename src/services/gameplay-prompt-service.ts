...
export function reviewGameplayInCodex(assetId: string, goal: string, seconds: number, report?: VisualReport) {
  ...
  const prompt = `${gameplayPrompt(project.id, assetId, goal, seconds, report)} Show it in Tools → Automatic Cuts for me to review before applying.`;
...
export async function generateGameplayInCodex(assetId: string, goal: string, seconds: number, mode: GameplayResult, trackId: string, report?: VisualReport) {
  ...
  if(!project?.assets.some(a => a.id === assetId && a.kind === 'video')) throw new Error('Choose a video first.');
  ...
  const result = mode === 'apply'
    ...
    : 'Show the saved proposal in Tools → Automatic Cuts for review. Do not change the timeline until I click Apply cut or explicitly ask you to apply it.';
