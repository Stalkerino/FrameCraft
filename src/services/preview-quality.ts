...
export function playbackSource(asset: Asset, quality: PreviewQuality, previews: MediaPreviews, rejected: Record<string, boolean>): PlaybackSource {
  if(asset.kind !== 'video' || quality === 'high' && !rejected[asset.id] && supportsOriginal(asset)) return {assetId: asset.id, src: asset.src, mode: 'original', width: asset.width, height: asset.height, status: 'ready'};
  const preview = previews[mediaPreviewKey(asset.id, quality)];
  const src = preview?.src || (quality === 'performance' ? asset.previewSrc : undefined);
  return {assetId: asset.id, src: preview?.status === 'ready' ? src : undefined, mode: quality, width: preview?.width, height: preview?.height, status: preview?.status === 'ready' && !src ? 'idle' : preview?.status ?? 'idle'};
}
