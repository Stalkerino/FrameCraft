...
import {emptyAgentSession, type AgentModelSettings, type AgentReply, type AgentSession} from '../../shared/agent';
...
export const useAgent = create<AgentState>((set, get) => {
  ...
  return {
    ...
    sendPrompt: (text, validate) => act(async () => {
      if(get().session.status === 'working') throw new Error('Wait for Codex to finish, or stop its current response.');
