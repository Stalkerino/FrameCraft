...
export function GameplayPanel() {
  ...
  return <div className="assist-section"><div className="assist-hint"><Eye size={22}/><strong>Automatic Cuts</strong><p>Tell {agentName} what to keep. It inspects the recording and creates cuts through the existing chat. Works with gameplay, devlogs, demonstrations and other footage. No narration needed.</p></div>{!asset ? <p className="field-help">Import a video to begin. No speech or transcription is needed.</p> : <>
    ...
    <p className="field-help">Starts or resumes your existing {agentName} session and sends the request immediately. Follow its analysis, approvals and answers in the chat. {agentName === 'Ollama' ? 'This uses your configured Ollama server.' : `This uses your ${agentName} session.`}</p>{generating && <p role="status" className="field-help">{agentName} is working — follow progress in the right-hand chat.</p>}{agentError && <p role="alert" className="agent-inline-error">{agentError}</p>}
    <div className="assist-section-title"><strong>Optional: inspect footage yourself</strong></div><p className="field-help">Scan creates a visual map only. Use Generate above to have {agentName} choose and create the cuts.</p>
    ...
    {preview !== null && <SourcePreview asset={asset} start={preview} end={Math.min(asset.duration, preview + 8)}/>}<Button icon={<Sparkles size={15}/>} disabled={active || !Number.isFinite(seconds) || seconds < 5} onClick={() => reviewGameplayInCodex(asset.id, goal, seconds, report)}>Review in {agentName}</Button><p className="field-help">Opens a request in your existing {agentName} chat. Send it there to start visual review. Source frames are sampled; short events need closer inspection.</p>
