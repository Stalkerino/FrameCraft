...
export function GameplayPanel() {
  ...
  return <div className="assist-section"><div className="assist-hint"><Eye size={22}/><strong>Gameplay cut builder</strong><p>Tell Codex what to keep. It inspects the recording and creates cuts through the existing chat. No narration needed.</p></div>{!asset ? <p className="field-help">Import a gameplay video to begin. No speech or transcription is needed.</p> : <>
    <Field label="Gameplay source"><select aria-label="Gameplay source" value={asset.id} onChange={event => {useAnalysis.setState({assetId: event.target.value}); setSheet(null); setPreview(null);}}>{videos.map(video => <option key={video.id} value={video.id}>{video.name}</option>)}</select></Field><Field label="What should the edit show?"><textarea aria-label="Gameplay editing goal" rows={3} placeholder="Show the new movement mechanics. Keep complete demonstrations; skip menus and repeated attempts." value={goal} onChange={event => setGoal(event.target.value)}/></Field><Field label="Target length (seconds)"><input aria-label="Gameplay target duration" type="number" min={5} value={seconds} onChange={event => setSeconds(Number(event.target.value))}/></Field>
    <Field label="AI result"><select aria-label="Gameplay AI result" value={mode} onChange={event => setMode(event.target.value as GameplayResult)}><option value="apply">Generate and apply cuts</option><option value="review">Generate a proposal for review</option></select></Field>
