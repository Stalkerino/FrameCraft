...
export function AgentConversation() {
  ...
  return <div className="agent-conversation"><AgentApprovalToggle/><div className="agent-conversation__scroll" ref={viewport} onScroll={() => {const el = viewport.current!; stick.current = el.scrollHeight - el.scrollTop - el.clientHeight < 80;}}>
    {!entries.length && <div className="agent-welcome"><div className="codex-orb"><Sparkles size={23}/></div><h3>Let’s make your next edit.</h3><p>Ask Codex to arrange footage, write titles, or build a transition. Changes appear on your timeline.</p></div>}
    {['idle', 'error', 'starting'].includes(session.status) && <Button className="agent-start" variant="primary" icon={starting ? <LoaderCircle size={15} className="spin"/> : <Sparkles size={15}/>} disabled={pending || starting || !online} onClick={() => void start()}>{starting ? 'Starting Codex…' : session.status === 'error' ? 'Reconnect Codex' : 'Start Codex'}</Button>}
    ...
    {session.status === 'working' && <p className="agent-working" role="status"><LoaderCircle size={13} className="spin"/>{session.requests.length ? 'Waiting for your response' : 'Codex is working…'}</p>}
