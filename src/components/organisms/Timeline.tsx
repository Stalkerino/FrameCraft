...
export function Timeline() {
  ...
  useEffect(() => {setSettingsId(null); setClearId(null);}, [project?.id]);
  ...
  const width = Math.max(900, (duration + Math.round(fps * 3)) * ppf); const tracks = project ? projectTracks(project) : [];
  ...
  const seek = (clientX: number) => useEditor.getState().seekTo(Math.min(duration - 1, timelineFrame(clientX)));
  const fit = () => useEditor.setState({zoom: Math.max(.01, Math.min(4, ((scroll.current?.clientWidth ?? 1440) - LABEL_WIDTH) / ((duration / fps + 3) * 48)))});
  return <section className="timeline" aria-label="Video timeline" onContextMenu={event => {const target = (event.target as HTMLElement).closest<HTMLElement>('[data-clip-id]'); if(!target?.dataset.clipId || !project) return; event.preventDefault(); const clip = project.clips.find(c => c.id === target.dataset.clipId); if(!clip) return; useEditor.setState({selectedId: clip.id, selectedTrackId: clip.trackId ?? clip.track, playing: false}); setContext({clipId: clip.id, x: event.clientX, y: event.clientY});}} tabIndex={-1} onPointerDownCapture={event => {if(event.target instanceof Element && !event.target.closest('button, input, [data-clip-id]')) event.currentTarget.focus({preventScroll: true});}}><TimelineToolbar project={project} onFit={fit}/>
    <div className="timeline-scroll" ref={scroll}><div className="timeline-content" style={{width: width + LABEL_WIDTH, minWidth: '100%'}}>
      <div className="timeline-ruler"><div className="track-label ruler-label">TRACKS <span>{fps} FPS</span></div><div className="ruler-area" style={{width}} onPointerDown={e => {seek(e.clientX); e.currentTarget.setPointerCapture(e.pointerId);}} onPointerMove={e => {if(e.buttons === 1) seek(e.clientX);}}>{Array.from({length: Math.ceil(width / (tick * ppf))}, (_, i) => <span key={i} className="ruler-tick" style={{left: i * tick * ppf}}>{formatTime(i * tick, fps)}</span>)}</div></div>
      {rows.map(row => <div key={row.id} data-track-id={row.id} className={`timeline-row timeline-row--${row.type} ${row.hidden ? 'timeline-row--hidden' : ''} ${dragTarget === row.id ? 'timeline-row--target' : ''}`} style={{height: row.height}}><TrackHeader track={row} count={row.clips.length} selected={selectedTrackId === row.id} busy={busy} onClear={() => {useEditor.setState({playing: false, error: null}); setClearId(row.id);}} onSelect={() => useEditor.setState({selectedTrackId: row.id, selectedId: null})} onSettings={() => setSettingsId(row.id)}/><div className="track-area" data-track-id={row.id} data-track-type={row.type} style={{width, backgroundSize: `${tick * ppf}px 100%`}} onPointerDown={e => {if(e.target === e.currentTarget) {useEditor.setState({selectedId: null, selectedTrackId: row.id}); seek(e.clientX);}}} onDragOver={e => {if(e.dataTransfer.types.some(type => ['application/framecraft-asset', 'application/framecraft-preset'].includes(type))) e.preventDefault();}} onDrop={e => {
      ...
      }}>{row.slots.map(({clip, lane}) => <TimelineClip key={clip.id} clip={clip} asset={project?.assets.find(a => a.id === clip.assetId)} pixelsPerFrame={ppf} top={8 + lane * (row.itemHeight + 4)}/>)}{!row.clips.length && <span className="track-placeholder">{row.type === 'audio' ? 'Drop audio here' : row.type === 'text' ? 'Add text or graphics here' : 'Drop footage or graphics here'}</span>}</div></div>)}
      {!rows.length && <p className="timeline-empty">Add a track to begin.</p>}
      <div className="playhead" style={{left: LABEL_WIDTH + frame * ppf, height: rows.reduce((sum, r) => sum + r.height, 30)}}><div className="playhead__handle" onPointerDown={e => {e.preventDefault(); e.currentTarget.setPointerCapture(e.pointerId);}} onPointerMove={e => {if(e.buttons === 1) seek(e.clientX);}}/></div>
    </div></div><div className="timeline-footer"><span><span className="status-dot"/> Local workspace <span className="footer-separator">/</span> Autosave enabled</span><span><kbd>Space</kbd> Play / pause <span className="footer-separator">·</span><kbd>C</kbd> Cut · <kbd>V</kbd> Select · <kbd>S</kbd> Split <span className="footer-separator">·</span><kbd>Ctrl C / V / D</kbd> Copy / paste / duplicate · <kbd>Ctrl Z / Y</kbd> Undo / redo</span><span>{formatTime(duration, fps)} total</span></div>{settingsId && <TrackSettingsDialog id={settingsId} onClose={() => setSettingsId(null)}/>} {clearId && <ClearTrackDialog id={clearId} onClose={() => setClearId(null)}/>} {context && <TimelineContextMenu {...context} onClose={closeContext}/>}</section>;
}
