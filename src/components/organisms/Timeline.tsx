...
export function Timeline() {
  ...
  return <section className="timeline" aria-label="Video timeline" onContextMenu={event => {const target = (event.target as HTMLElement).closest<HTMLElement>('[data-clip-id]'); if(!target?.dataset.clipId || !project) return; event.preventDefault(); const clip = project.clips.find(c => c.id === target.dataset.clipId); if(!clip) return; useEditor.setState({selectedIds: timelineSelection(useEditor.getState()).includes(clip.id) ? [...timelineSelection(useEditor.getState())] : [clip.id], selectedId: clip.id, selectedTrackId: clip.trackId ?? clip.track, playing: false}); setContext({clipId: clip.id, x: event.clientX, y: event.clientY});}} tabIndex={-1} onPointerDownCapture={event => {if(event.target instanceof Element && !event.target.closest('button, input, [data-clip-id]')) event.currentTarget.focus({preventScroll: true});}}><TimelineToolbar project={project} onFit={fit}/>
