...
export function CodexPanel() {
  ...
  const local = session.provider === 'ollama'; const providerName = local ? 'Ollama' : 'Codex';
  ...
  const content = <>
    ...
    <div className="agent-connection-line"><span className={`connection-badge ${(local ? ['ready', 'working'].includes(session.status) : connected) ? 'connected' : ''}`}><span className="status-dot"/>{error ? 'Editor service unavailable' : local ? (session.status === 'ready' || session.status === 'working' ? 'Ollama · MCP connected' : 'Ollama not connected') : connected ? 'Codex connected' : 'Waiting for a Codex session'}</span><IconButton label={`Check ${providerName} connection`} disabled={checking} onClick={() => void refresh()}><RefreshCw size={12} className={checking ? 'spin' : ''}/></IconButton></div>
    ...
    <div className="agent-panel-content" role="tabpanel" aria-label={`${providerName} ${tab}`}>{tab === 'chat' ? <AgentConversation/> : tab === 'activity' ? <AgentActivityPanel/> : tab === 'settings' ? <div className="agent-settings-panel"><AgentProviderSettings/><AgentModelSettings/></div> : <AgentConnectionPanel status={status} error={error}/>}</div>
  </>;
