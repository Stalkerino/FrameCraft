...
export function SoundLibrary() {
  ...
  return <div className="sound-library"><p className="panel-description">Reusable whooshes, impacts and interface cues. Preview a sound, customize it, or add it at the playhead.</p>
    ...
    <div className="sound-library__toolbar"><Button icon={<Sparkles size={14}/>} onClick={ask}>Ask Codex</Button><Button disabled={actions.busy} icon={<Upload size={14}/>} onClick={() => input.current?.click()}>Import recipe</Button><input ref={input} type="file" accept=".json" hidden aria-label="Import sound recipe" onChange={event => {const file = event.target.files?.[0]; event.target.value = ''; if(file) void actions.importRecipe(file).then(saved => {if(saved) void library.refresh();});}}/></div>
