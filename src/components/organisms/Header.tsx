...
import {ProjectCareDialog} from './ProjectCareDialog';
...
export function Header() {
  ...
  const [careDialog, setCareDialog] = useState(false);
  const [clearDialog, setClearDialog] = useState(false);
  const [projectDialog, setProjectDialog] = useState<'new' | 'copy' | 'open' | null>(null);
  useEffect(() => {setCareDialog(false); setProjectDialog(null); setSettingsDialog(false); setExportDialog(false); setTimelineExportDialog(false); setClearDialog(false); setEditing(false);}, [snapshot?.project.id, snapshot?.project.sequenceId]);
  ...
  const desktop = desktopApi.available();
  return <header className={`topbar ${desktop ? 'topbar--desktop' : ''}`} data-tauri-drag-region={desktop ? true : undefined}>
    <div className="brand" data-tauri-drag-region={desktop ? true : undefined} aria-label="Framecraft Studio"><span className="brand__mark">f<span/></span><span>framecraft<span className="brand__suffix">studio</span></span></div>
    <div className="project-title"><FolderOpen size={15}/><div className="project-title__content">{editing ? <input autoFocus aria-label="Project name" defaultValue={snapshot?.project.name} maxLength={120} onBlur={e => {const name = e.target.value.trim(); if(name && name !== snapshot?.project.name) void useEditor.getState().execute([{type: 'project.rename', name}], 'Renamed project'); setEditing(false);}} onKeyDown={e => {if(e.key === 'Enter') e.currentTarget.blur();}}/> : <ProjectMenu key={snapshot?.project.id} name={snapshot?.project.name || 'Opening project'} disabled={!snapshot || busy} onNew={() => openProjects('new')} onOpen={() => openProjects('open')} onCopy={() => openProjects('copy')} onRename={() => setEditing(true)} onClear={() => setClearDialog(true)} onCare={() => {useEditor.setState({playing: false}); setCareDialog(true);}}/>}<span className="project-title__meta">{snapshot ? `${snapshot.project.width} × ${snapshot.project.height} · ${snapshot.project.fps} fps` : 'Local project'}</span></div></div>
    {desktop && <div className="topbar__drag-space" data-tauri-drag-region/>}
    <div className="topbar__actions">{snapshot?.recoveryNotice && <button className="project-recovery-notice" title={snapshot.recoveryNotice} onClick={() => setCareDialog(true)}>Project recovered · review backups</button>}<span className={`save-status ${!connected ? 'save-status--offline' : ''}`}>{busy ? <LoaderCircle className="spin" size={13}/> : <Check size={13}/>}<span>{!connected ? 'Connecting…' : busy ? 'Saving…' : 'All changes saved'}</span></span>
