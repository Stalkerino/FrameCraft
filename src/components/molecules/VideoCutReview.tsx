...
export function VideoCutReview({cut, project, asset}: {cut: VideoCut; project: Project; asset: Asset}) {
  ...
  return <div className="gameplay-review"><div className="assist-section-title"><strong>{cut.title}</strong><span className="match-badge">AI PROPOSAL</span></div><p className="field-help">{ids.length} shots · {Math.round(cut.shots.filter(shot => ids.includes(shot.id)).reduce((sum, shot) => sum + shot.end - shot.start, 0))} seconds selected</p>{selected && <SourcePreview asset={asset} start={selected.start} end={selected.end} seekKey={selected.id}/>}
