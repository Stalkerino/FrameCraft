import {addTimelineMarker, jumpTimelineMarker} from '../services/organization-actions';
...
export function useEditorKeyboard() {
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      if(key === 'm' && !event.altKey && (!modifier || event.shiftKey)) {event.preventDefault(); if(!event.repeat) {if(event.shiftKey) jumpTimelineMarker(modifier ? -1 : 1); else if(!state.busy) void addTimelineMarker();}}
      else if(modifier && !event.altKey && key === 'c' && state.selectedId && !window.getSelection()?.toString()) {event.preventDefault(); state.copyClip();}
      ...
      else if(modifier && key === 'g' && state.selectedId) {event.preventDefault(); if(!event.repeat && !state.busy) setClipRelationship('group', event.shiftKey);}
      else if((event.key === 'Delete' || event.key === 'Backspace') && state.selectedId) {event.preventDefault(); if(!state.busy) {if(event.shiftKey) rippleDeleteSelection(); else void state.removeSelected();}}
      ...
      else if(!modifier && !event.altKey && ['c', 'v'].includes(event.key.toLowerCase())) {event.preventDefault(); useEditor.setState({timelineTool: event.key.toLowerCase() === 'c' ? 'razor' : 'select', trimMode: 'trim'});}
