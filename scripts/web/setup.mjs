...
import {root, runtime, environment, readConfig, run, npmCli, findExecutable, fingerprint, mainModule} from '../install/runtime.mjs';
...
export async function setup({start = true} = {}) {
  ...
  try {
    ...
    if(process.arch === 'x64' && ['win32', 'linux'].includes(process.platform)) {
      const {installVulkan} = await import('../install/vulkan.mjs');
      await installVulkan();
    }
    ...
    console.log('Web setup complete. Next time use npm run web:launch.');
